...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, self.hidden_size, num_layers=1, bias=True, dropout=dropout_rate, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        package = pack_padded_sequence(X, source_lengths)
        out_encoder, tuple_h_c = self.encoder(package)
        enc_hiddens, len_enc = pad_packed_sequence(out_encoder)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        init_decoder_hidden = self.h_projection(torch.cat((tuple_h_c[0][0], tuple_h_c[0][1]), 1))
        init_decoder_cell = self.c_projection(torch.cat((tuple_h_c[1][0], tuple_h_c[1][1]), 1))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        hidden_, cell_ = self.decoder(Ybar_t, dec_state)
        dec_state = (hidden_, cell_)
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(hidden_, 2)))
        ...
        U_t = torch.cat((a_t, hidden_), dim=1)
