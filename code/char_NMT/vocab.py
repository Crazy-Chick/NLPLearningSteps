...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sentence in sents:
            ...
            for word in sentence:
                word_index = []
                word_index.append(self.start_of_word)
                for ch in word:
                    if ch in self.char_list:
                        word_index.append(self.char2id[ch])
                    else:
                        word_index.append(self.char_unk)
                word_index.append(self.end_of_word)
                sentence_index.append(word_index)
            word_ids.append(sentence_index)
        return word_ids        
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        char_index = pad_sents_char(self.words2charindices(sents))
        sents_var = torch.tensor(char_index, device=device)
