...
def pad_sents_char(sents, char_pad_token):
    ...
    for i in range(max_word_length):
        pad_word.append(char_pad_token)
    for sentence_id in range(len(sents)):
        for words_id in range(len(sents[sentence_id])):
            while (len(sents[sentence_id][words_id]) < max_word_length):
                sents[sentence_id][words_id].append(char_pad_token)
        while (len(sents[sentence_id]) < max_words):
            sents[sentence_id].append(pad_word)
    sents_padded = sents
